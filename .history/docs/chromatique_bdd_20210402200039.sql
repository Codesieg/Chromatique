...
CREATE TABLE IF NOT EXISTS `chromatique`.`mangas` (
  ...
  UNIQUE INDEX `id_UNIQUE` (`id` ASC),
  INDEX `fk_mangas_users1_idx` (`users_id` ASC),
  CONSTRAINT `fk_mangas_users1`
...
CREATE TABLE IF NOT EXISTS `chromatique`.`tomes` (
  ...
  `created_at` TIMESTAMP NULL,
  `updated_at` TIMESTAMP NULL,
  `mangas_id` INT UNSIGNED NOT NULL,
  PRIMARY KEY (`id`, `mangas_id`),
  UNIQUE INDEX `name_UNIQUE` (`tome_name` ASC),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC),
  INDEX `fk_tomes_mangas1_idx` (`mangas_id` ASC),
  CONSTRAINT `fk_tomes_mangas1`
...
CREATE TABLE IF NOT EXISTS `chromatique`.`chapters` (
  ...
  `updated_at` TIMESTAMP NULL,
  `tomes_id` INT UNSIGNED NOT NULL,
  PRIMARY KEY (`id`, `tomes_id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC),
  INDEX `fk_chapters_tomes1_idx` (`tomes_id` ASC),
  CONSTRAINT `fk_chapters_tomes1`
    FOREIGN KEY (`tomes_id`)
...
CREATE TABLE IF NOT EXISTS `chromatique`.`pages` (
  ...
  `chapters_id` INT UNSIGNED NOT NULL,
  PRIMARY KEY (`id`, `chapters_id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC),
  INDEX `fk_pages_chapters1_idx` (`chapters_id` ASC),
  CONSTRAINT `fk_pages_chapters1`
...
CREATE TABLE IF NOT EXISTS `chromatique`.`users_historys` (
  ...
  UNIQUE INDEX `id_UNIQUE` (`id` ASC),
  INDEX `fk_users_historys_users1_idx` (`users_id` ASC),
  INDEX `fk_users_historys_mangas1_idx` (`mangas_id` ASC),
  INDEX `fk_users_historys_tomes1_idx` (`tomes_id` ASC),
  INDEX `fk_users_historys_chapters1_idx` (`chapters_id` ASC),
  INDEX `fk_users_historys_pages1_idx` (`pages_id` ASC),
  CONSTRAINT `fk_users_historys_users1`
    FOREIGN KEY (`users_id`)
    REFERENCES `chromatique`.`users` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_users_historys_mangas1`
    FOREIGN KEY (`mangas_id`)
    REFERENCES `chromatique`.`mangas` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_users_historys_tomes1`
    FOREIGN KEY (`tomes_id`)
    REFERENCES `chromatique`.`tomes` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_users_historys_chapters1`
    FOREIGN KEY (`chapters_id`)
    REFERENCES `chromatique`.`chapters` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_users_historys_pages1`
