...
CREATE TABLE IF NOT EXISTS `chromatique`.`tomes` (
  ...
  `updated_at` TIMESTAMP NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `name_UNIQUE` (`tome_name` ASC) VISIBLE,
  INDEX `id_manga_idx` (`id_manga` ASC) VISIBLE,
  INDEX `id_uploader_idx` (`id_uploader` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `chromatique`.`chapters` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) VISIBLE,
  INDEX `id_manga_idx` (`id_tome` ASC) VISIBLE,
  CONSTRAINT `id_tome`
    FOREIGN KEY (`id_tome`)
...
CREATE TABLE IF NOT EXISTS `chromatique`.`pages` (
  ...
  PRIMARY KEY (`id`),
  INDEX `id_chapter_idx` (`id_chapter` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `chromatique`.`users_historys` (
  ...
  INDEX `id_chapter_idx` (`id_chapter` ASC) VISIBLE,
  INDEX `id_tome_idx` (`id_tome` ASC) VISIBLE,
  INDEX `id_manga_idx` (`id_manga` ASC) VISIBLE,
  INDEX `id_pages_idx` (`id_page` ASC) VISIBLE,
  CONSTRAINT `id_user`
    FOREIGN KEY (`id_user`)
    REFERENCES `chromatique`.`users` (`id`)
    ON DELETE CASCADE
