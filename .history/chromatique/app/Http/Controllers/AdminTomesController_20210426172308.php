...
class AdminTomesController extends Controller
{
    ...
    public function read($id)
    {
        ...
        ->get();
// dd($listTomes);
        $listTomes->mangaName = $this->getMangaName($id);

        dump($listTomes);

        return view('admin/tomes/detail', compact ('listTomes'));
    }

    public function getMangaName($id) {
        return Mangas::select('manga_name')
                        ->where('id', $id)
                        ->get();
    }
    /**
