...
class AdminTomesController extends Controller
{
    ...
    {
        // dd($newManga);
        $allDir = Storage::disk('local')->allFiles('public/mangas/' . $newManga);
        
            // dd($allDir);
            $AllNewTomes = [];
        foreach ($allDir as $tome) {
            $newTome = explode("/", $tome);
            // dd($newTome);
            if (in_array($tome[3], $AllNewTomes, true)) {
                echo "Tome déja présent </br>";
            } else {
                if (is_dir(Storage::disk('local')->allFiles('public/mangas/' . $newManga . $newTome[3])) {
        ...
        dd($AllNewTomes);
            ...
            foreach ($allDir as $newTome) { 
                $tome = Tomes::firstOrCreate(
                    ['tome_number' => $tomeNumber],
                    ['tome_path' => $newManga .".jpg",
                    'tome_number' => "/". $newManga,
