        ...
        // Boucle sur les pages
            if($request->hasfile('pages'))
            {
                foreach ($request->file('pages') as $page) {
                    $pagePath = $newMangaDirectory . '/' . $tomeName . '/' . $page->getClientOriginalName();        
                    // $pageNumber = explode('-', $pagePath);

                $newPage = Storage::disk('mangas')->putFileAs($newMangaDirectory . $tomeName, $page , $page->getClientOriginalName());

                    dd(Storage::path($newPage));
                    // Création des pages du tome :
                    Pages::firstOrCreate(
                        ['tome_id'=> $newTome->id, 'page_number' => $pagePath],
